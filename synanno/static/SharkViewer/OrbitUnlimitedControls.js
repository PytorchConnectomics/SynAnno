...
// Code from Janelia CompSci https://github.com/JaneliaSciComp/three-orbit-unlimited-controls
...
class OrbitUnlimitedControls extends THREE.EventDispatcher {
  ...
  rotate = (x, y) => {
    ...
    if (this.state === State.INDETERMINATE) {
      ...
      if (distance > ClickThreshold) {
        ...
        const tr = new THREE.Matrix4().makeTranslation(
          this.target.x,
          this.target.y,
          this.target.z
        );
    ...
    if (this.state === State.ROTATING) {
      ...
      let theta =
        -delta.y * (Math.PI / this.domElement.clientHeight) + Math.PI / 2;
  ...
  dolly = (x, y) => {
    ...
    if (this.state === State.DOLLYING) {
      ...
      const distForFullDrag = Math.min(
      ...
      // distance.
      if (
        this.minDistance <= distToTarget + delta &&
  ...
  pan = (x, y) => {
    ...
    if (this.state === State.PANNING) {
      ...
      const halfHeightWorld =
        Math.tan((this.object.fov / 2) * (Math.PI / 360)) * distToTarget;
  ...
  fixUp = () => {
    ...
    const side = new THREE.Vector3()
      .crossVectors(view, this.object.up)
      .normalize();
