...
class SynANNODataset(Dataset):
    def __init__(
        self,
        materialization_table_path,
        source_url,
        target_url,
        bucket_secret_json_path,
        local_dir,
        seed=42,
        transform=None,
        coord_order=["x", "y", "z"],
        coord_resolution_source=["4", "4", "33"],
        coord_resolution_target=["8", "8", "33"],
        crop_sizes: dict = {"crop_size_x": 128, "crop_size_y": 128, "crop_size_z": 16},
        ...
        self.materialization_table_path = materialization_table_path
        self.source_url = source_url
        self.target_url = target_url
        self.bucket_secret_json_path = bucket_secret_json_path
        self.local_dir = local_dir
        self.coord_order = coord_order
        self.coord_resolution_source = coord_resolution_source
        self.coord_resolution_target = coord_resolution_target
        self.transform = transform
        self.seed = seed
        self.crop_sizes = crop_sizes
        self.local_execution = local_execution

    def create_dataset(self, n_instances):
        # Connect to CloudVolumes and get metadata
        self.source_cv, self.target_cv = self.connect_to_cloudvolumes(
            self.source_url, self.target_url, self.bucket_secret_json_path
        )

        # Get volume dimensions and scale
        self.vol_dim, self.vol_dim_scaled, self.scale = self.cloudvolume_metadata(
            self.source_cv,
            self.target_cv,
            self.coord_order,
            self.coord_resolution_source,
            self.coord_resolution_target,
        )

        # Select random instances
        self.instance_keys, self.bbox_dict = self.select_random_instances(
            n_instances, self.materialization_table_path, self.seed
        )

        # Download subvolumes
        self.vol_dim = (
            {c: v for c, v in zip(self.coord_order, self.vol_dim)}
            if not isinstance(self.vol_dim, dict)
            else self.vol_dim
        )
        self.download_subvolumes(
            self.instance_keys,
            self.source_cv,
            self.target_cv,
            self.local_dir,
    ...
    @staticmethod
    def select_random_instances(n, table_name, seed):
        """
        ...
        df_sampled = df.sample(n, random_state=seed).reset_index()
    ...
    def connect_to_cloudvolumes(self, source_url, target_url, bucket_secret_json_path):
        """
    ...
    def cloudvolume_metadata(
        ...
        Args:
            ...
            coord_order (list): Order of coordinates (e.g., ['x', 'y', 'z']).
    ...
    def download_subvolumes(
        ...
        for key in instance_keys:
            ...
            bbox, padding = self.get_bbox_from_instance_data(
                instance_data,
                coord_order,
                crop_sizes,
                volume_dimensions,
            ...
            gt_subvol_pad = np.pad(
                gt_subvol, padding, mode="constant", constant_values=0
            )
            ...
            gt_subvol_pad_augmented = self.generate_training_data(
                gt_subvol_pad, coord_order, crop_size_z=crop_sizes["crop_size_z"]
            )
            ...
            target_subvol_pad = np.concatenate(
                (source_subvol_pad, gt_subvol_pad), axis=3
            )
            ...
            if not self.local_execution:
                ...
                self.upload_to_bucket(source_blob_name, source_subvol_pad)
                self.upload_to_bucket(target_blob_name, target_subvol_pad)
            else:
                np.save(
                    os.path.join(
                        source_dir, f"source_{materialization[key]['index']}.npy"
                    ),
                    source_subvol_pad,
                )
                np.save(
                    os.path.join(
                        target_dir, f"target_{materialization[key]['index']}.npy"
                    ),
                    target_subvol_pad,
                )

    def generate_training_data(
        self, gt_subvol: np.ndarray, coord_order: List[str], crop_size_z: int = 16
    ):
        """
        ...
        """
        seed_layers = self.get_seed_layers(crop_size_z)
        augmented_subvol = self.augment_target_volume(
            gt_subvol, seed_layers, coord_order
        )
    ...
    def get_bbox_from_instance_data(
        ...
        Args:
            ...
            coord_order (List[str]): The order of the coordinates (e.g., ['x', 'y', 'z']).
        ...
        adjusted_bbox, pad = self.calculate_crop_pad(
        ...
        if coord_order != ["x", "y", "z"]:
            bbox_min = [bbox_min[coord_order.index(coord)] for coord in ["x", "y", "z"]]
            bbox_max = [bbox_max[coord_order.index(coord)] for coord in ["x", "y", "z"]]
    ...
    def get_seed_layers(self, z_crop_size: int) -> Set[int]:
        """
    ...
    def augment_target_volume(
        self, gt_subvol: np.ndarray, seed_layers: Set[int], coord_order: List[str]
    ) -> np.ndarray:
        """
    ...
    def calculate_crop_pad(
        self, bbox_3d: List[int], volume_shape: List[int], pad_z: bool = False
        ...
        pad = [
            [bbox[i] - bbox_3d[i], bbox_3d[i + 1] - bbox[i + 1]] for i in range(0, 6, 2)
        ]
    ...
    def visualize_data(self, instance: Union[np.ndarray, str], slice: int = 8) -> None:
        if isinstance(instance, str):
            data = np.load(instance)
        else:
            data = instance
    ...
    def __len__(self):
        ...
        return len(self.instance_keys)

    def __getitem__(self, idx):
        # check that self.instance_keys exists
        if not hasattr(self, "instance_keys"):
            raise ValueError(
                "Instance keys not found. Please run create_dataset() before accessing the dataset."
            )

        # check if the index is valid
        if idx >= len(self.instance_keys):
            raise IndexError(
                f"Index {idx} is out of bounds. Please select an index between 0 and {len(self.instance_keys) - 1}."
            )

        # Retrieve instance key and download subvolumes
        instance_key = self.instance_keys[idx]

        # Determine file paths based on local or GCP execution
        if self.local_execution:
            source_file = os.path.join(
                self.local_dir,
                "source",
                f"source_{self.bbox_dict[instance_key]['index']}.npy",
            )
            target_file = os.path.join(
                self.local_dir,
                "target",
                f"target_{self.bbox_dict[instance_key]['index']}.npy",
            )
        else:
            source_blob_name = (
                f"source/source_{self.bbox_dict[instance_key]['index']}.npy"
            )
            target_blob_name = (
                f"target/target_{self.bbox_dict[instance_key]['index']}.npy"
            )
            source_file = self.download_from_bucket(source_blob_name)
            target_file = self.download_from_bucket(target_blob_name)
