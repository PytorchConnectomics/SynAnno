...
def upload_to_bucket(blob_name, data):
    """Uploads numpy array data to the bucket."""
    client = storage.Client()
    bucket = client.bucket("synanno")
    blob = bucket.blob(blob_name)

    # Convert numpy array to bytes and upload
    np_data_bytes = np.ascontiguousarray(data).tobytes()
    blob.upload_from_string(np_data_bytes, content_type="application/octet-stream")
...
def download_from_bucket(blob_name):
    ...
    data = np.frombuffer(
        np_data_bytes, dtype=np.float64
    )  # Change dtype as per your data
    return data
